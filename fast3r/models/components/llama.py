...
# source: https://raw.githubusercontent.com/Lightning-AI/pytorch-lightning/refs/heads/master/examples/pytorch/tensor_parallel/model.py
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    if freqs_cis.ndim == 2: # (seqlen, dim)
        assert freqs_cis.shape == (seqlen, x.shape[-1])
        shape = [d if i == 1 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)]  # (1, seqlen, 1, dim)
        return freqs_cis.view(*shape)  # (1, seqlen, 1, dim)
    elif freqs_cis.ndim == 3:  # (bs, seqlen, dim)
        assert freqs_cis.shape[0] == x.shape[0]
        assert freqs_cis.shape[1:] == (seqlen, x.shape[-1])
        return freqs_cis.unsqueeze(2)  # (bs, seqlen, 1, dim)
    else:
...
class Attention(nn.Module):
    ...
    Args:
        ...
        is_causal (bool): Flag to indicate if the attention is causal.
...
class TransformerBlock(nn.Module):
    ...
    Args:
        ...
        dim (int): Dimension size of the model.
        multiple_of (int): Value to ensure hidden dimension is a multiple of this value.
        ffn_dim_multiplier (float): Custom multiplier for hidden dimension.
        n_layers (int): Number of layers in the model.
        is_causal (bool): Flag to indicate if the attention is causal.
        norm_eps (float): Epsilon value for layer normalization.
        depth_init (bool): Flag to indicate if the layer initialization uses the layer ID.
    ...
    def __init__(self, layer_id: int, n_heads: int, n_kv_heads: Optional[int], dim: int, multiple_of: int,
        ...
        self.n_heads = n_heads
        self.dim = dim
        self.attention = Attention(n_heads=n_heads,
